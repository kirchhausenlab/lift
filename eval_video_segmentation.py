...
def eval_video_tracking_davis(args, model, transform, frame_list, video_dir, first_seg, seg_ori, color_palette):
    ...
    os.makedirs(os.path.join(args.output_dir, f'davis_vidseg_224_layer{args.layer}'), exist_ok=True)
    output_dir = os.path.join(args.output_dir, f'davis_vidseg_224_layer{args.layer}')
    ...
    frame1_feat = extract_feature(args, model, transform, frame1, patch_size=args.patch, imsize=args.imsize).T #  dim x h*w
...
def label_propagation(args, model, transform, frame_tar, list_frame_feats, list_segs, mask_neighborhood=None):
    ...
    feat_tar, h, w = extract_feature(args, model, transform, frame_tar, return_h_w=True, patch_size = args.patch, imsize=args.imsize)
...
def extract_feature(args, model, transform, frame, return_h_w=False, patch_size=16, imsize=224):
    ...
    with torch.no_grad():
        ...
        out = model.extract_descriptors(frame.unsqueeze(0).cuda())
    if args.lift_path is not None:
        h, w = 2*int(frame.shape[1] / patch_size), 2*int(frame.shape[2] / patch_size)
    else:
        if(args.patch!=args.stride):
            h = (args.imsize - args.patch)//args.stride + 1
            w = (args.imsize - args.patch)//args.stride + 1
        else:
