...
class Dataset:
    def __init__(self, dataset_name, dataset_set, remove_hards, resize=None, dataroot="datasets"):
        """
        ...
        if dataset_name == "VOC07":
            self.root_path = os.path.join(self.dataroot, "VOC2007")
            self.year = "2007"
        elif dataset_name == "VOC12":
            self.root_path = os.path.join(self.dataroot, "VOC2012")
            self.year = "2012"
        elif dataset_name == "COCO20k":
            self.year = "2014"
            self.root_path = os.path.join(self.dataroot, f"COCO/images/{dataset_set}{self.year}")
            self.sel20k = os.path.join(self.dataroot, 'coco_20k_filenames.txt')
            # JSON file constructed based on COCO train2014 gt 
            self.all_annfile = os.path.join(self.dataroot, "COCO/annotations/instances_train2014.json")
            self.annfile = os.path.join(self.dataroot, "instances_train2014_sel20k.json")
        ...
        if "VOC" in dataset_name:
            ...
            if(resize is not None):
                ...
                self.dataloader = CocoDetection(
                    self.root_path, annFile=self.annfile, transform=transform_resize
                )
            else:
                self.dataloader = CocoDetection(
    ...
    def load_image(self, im_name):
        ...
        if "VOC" in self.dataset_name:
            image = skimage.io.imread(os.path.join(self.dataroot, f"VOC{self.year}/VOCdevkit/VOC{self.year}/JPEGImages/{im_name}"))
            ...
            image = skimage.io.imread(os.path.join(self.dataroot, f"COCO/images/train2014/{im_name}"))
        else:
    ...
    def get_hards(self):
        hard_path = os.path.join(self.dataroot, "hard_%s_%s_%s.txt" % (self.dataset_name, self.set, self.year))
...
def select_coco_20k(sel_file, all_annotations_file):
    ...
    with open(os.path.join(self.dataroot, "instances_train2014_sel20k.json"), "w") as outfile:
