...
def main(args):
    ...
    output_dir = args.output_dir +  '/' + args.model_type + '_' + str(args.lr) + '_' + args.loss
    if(args.augment):
    ...
    train_dataset = torch.utils.data.Subset(train_dataset, torch.randperm(len(train_dataset)).tolist())
    dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=16)
    ...
    for epoch in range(epoch_start, args.epochs):
        ...
        for batch_index, image_batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                if args.my_extractor:
                    ...
                    feat1 = extractor.extract_descriptors(image_batch, args.layer, args.facet)[:,0,:,:]
                    feat2 = extractor.extract_descriptors(image_batch[:, :, ::2, ::2], args.layer, args.facet)[:,0,:,:]
                    feat3 = extractor.extract_descriptors(image_batch[:, :, ::4, ::4], args.layer, args.facet)[:,0,:,:]
            ...
            if(args.loss == 'cosine'):
                ...
                loss = criterion(gen_1, feat1, torch.ones(gen_1.shape[0]).to(device)) + criterion(gen_2, feat2, torch.ones(gen_2.shape[0]).to(device))
            else:
                loss = criterion(gen_1, feat1) + criterion(gen_2, feat2)
            
            print('Epoch [{}/{}], Batch [{}/{}], Loss: {:.4f}'.format(epoch + 1, args.epochs, batch_index + 1, len(dataloader), loss.item()),flush=True)
        ...
        if (epoch + 1) % args.save_every == 0:
            torch.save(lift.state_dict(), os.path.join(output_dir, 'lift_{}.pth'.format(epoch + 1)))
...
def parse_args():
    ...
    ### LiFT TRAINING ###
    parser.add_argument('--imsize', default=448, type=int, help='largest image size during training')
    ...
    parser.add_argument('--loss', default='cosine', type=str, help="choose from l1, l2, cosine")
    parser.add_argument('--output_dir', default='lift_imagenet_trains', help='dir to save checkpoints to')
    ### DATASET ###
    parser.add_argument('--dataroot', default='/path/to/imagenet/train')
    parser.add_argument('--seed', type=int, default=0, help='random seed')
    ### OTHER ###
    parser.add_argument('--my_extractor', action='store_true', help='Enable loading of a custom feature extractor instead of a built in one. Must be implemented in my_extractor.')
    ###
